...
def creat_data(data_size, attribute_num, false_percent):
        ...
        mu_white, sigma_white = 0, 0.001 # Define average and variance or normal distribution as white noise
        data_set_true = []
        data_set_false = []
        data_set_origin = []
        data_set = []
        ...
        t = np.zeros(sec_size)
        for t_count in range (0, sec_size):
                        t[t_count] = 0.5 + t_count * 1/sec_size

        #t[sec_size-1] = 0.5 + (sec_size-1)/sec_size
        ...
        for i in range(0, int(data_size*(1-false_percent))):
                ...
                for k in range(sec_num-1, sec_num):

                        for j in range (k*sec_size, (k+1)*sec_size):
                                noise = np.random.normal(mu_white, sigma_white, 1)      # Gaussian noise
                                x[j] = 1/t[j-k*sec_size] + noise                        # x = 1/t + noise
                        if x[(k+1)*sec_size-1] - x[k*sec_size] < 0:
                                y_sec[k] = 0
                        else: y_sec[k] = 1
        ...
        for i in range(0,int(data_size*false_percent)):
                ...
                if y_sec[3] == 0:
                        for j in range (3*sec_size, (3+1)*sec_size):
                                noise = np.random.uniform(-0.5, 0.5, 1)      # random noise
                                x[j] = 1/t[j-3*sec_size] + noise

                if y_sec[3] == 1:
                        for j in range(3*sec_size, (3+1)*sec_size):
                                noise = np.random.uniform(-0.5, 0.5, 1)
                                x[j] = t[j-3*sec_size] * t[j-3*sec_size] + noise        # x = t^2 + noise
...
def main():
        ...
        np.savetxt("train_file_curve.csv", train_data, delimiter=",")
       
        # produce test dataset
        test_data = creat_data(50000, 200, 0.1)
        np.savetxt("test_file_curve.csv", test_data, delimiter=",")
                       
        # produce even test dataset
        test_data = creat_data(50000, 200, 0.5)
        np.savetxt("test_file_curve_even.csv", test_data, delimiter=",")
